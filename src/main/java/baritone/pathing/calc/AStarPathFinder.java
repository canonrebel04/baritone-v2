...
public final class AStarPathFinder extends AbstractNodeCostSearch {
    ...
    protected Optional<IPath> calculate0(long primaryTimeout, long failureTimeout) {
        ...
        while (!openSet.isEmpty() && numEmptyChunk < pathingMaxChunkBorderFetch && !cancelRequested) {
            ...
            // ⚡ Bolt: Hoist invariant chunk coordinate calculations outside the innermost movement loop
            // This avoids recalculating the bitshifts for the base node millions of times during pathfinding.
            int currentNodeChunkX = currentNode.x >> 4;
            int currentNodeChunkZ = currentNode.z >> 4;
            for (Moves moves : allMoves) {
                int newX = currentNode.x + moves.xOffset;
                int newZ = currentNode.z + moves.zOffset;
                if ((newX >> 4 != currentNodeChunkX || newZ >> 4 != currentNodeChunkZ) && !calcContext.isLoaded(newX, newZ)) {
