...
**Learning:** `BinaryHeapOpenSet` is a hot-path data structure running millions of times per long Baritone path. In `update` and `removeLowest`, the nodes shift positions around heavily. A naive swap does array stores on every loop iteration. Doing a 'half-exchange' optimization saves hundreds of thousands of redundant memory writes by caching the target value and just dragging other nodes over it until we find its real resting place.
**Action:** Replaced iterative array stores with terminal deferred assignments in `BinaryHeapOpenSet.java`.
