...
**Learning:** In A* pathfinding, the `BinaryHeapOpenSet` priority queue operations (sift-up and sift-down) are executed millions of times. The standard swap-based loop involves 3 array assignments and multiple heap position updates per step. By adopting a "half-exchange" optimization, we defer `array[index] = val` and `val.heapPosition = index` until the end of the loop, executing them only once when the final index is found. This significantly reduces memory traffic and CPU overhead inside the critical inner loop.
**Action:** Replaced sift-up (`update`) and sift-down (`removeLowest`) implementations with half-exchange logic in `BinaryHeapOpenSet`.
