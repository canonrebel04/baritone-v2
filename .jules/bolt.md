...
**Learning:** In A* pathfinding algorithms which perform millions of sift-up/sift-down operations on priority queues, implementing a half-exchange pattern by deferring the target node and position assignments until the end of the `update` (sift-up) and `removeLowest` (sift-down) loops reduces redundant memory writes by roughly half.
**Action:** Always look for opportunities to apply half-exchange patterns inside inner loops of performance-critical data structures.
