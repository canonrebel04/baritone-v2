...
## 2025-05-21 - BinaryHeapOpenSet Half-Exchange Pattern
**Learning:** `BinaryHeapOpenSet` sift-up (`update`) and sift-down (`removeLowest`) originally used full exchanges (swaps) inside the loop. Deferring the node placement and heapPosition update to outside the loop ("half-exchange") reduces memory stores in the innermost pathing loops.
**Action:** Refactored `update` and `removeLowest` to use the half-exchange pattern to minimize array assignments and field updates.
