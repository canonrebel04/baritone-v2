...
## 2026-06-05 - Priority Queue Half-Exchange Optimization
**Learning:** In the `BinaryHeapOpenSet` of the A* algorithm, sift-up (`update`) and sift-down (`removeLowest`) operations historically used full swaps, repeatedly writing array elements and updating node `heapPosition` properties at every step. This resulted in significant, redundant memory stores on the hottest path of the algorithm. Deferring the final store and node position update until after the loop completes—known as a "half-exchange"—drastically reduces these redundant writes.
**Action:** Implemented half-exchanges in `BinaryHeapOpenSet.java`, removing `val` intermediate writes and moving `array[index] = val; val.heapPosition = index;` to the end of the loop.
