...
## 2026-06-05 - BinaryHeapOpenSet Half-Exchange Priority Queue
**Learning:** In Baritone's A* pathfinding, the `BinaryHeapOpenSet` Priority Queue sift-up and sift-down operations heavily access arrays. The standard swap strategy continuously overwrites the inserted/bubbled value (`val`). Using a 'half-exchange' optimization defers both the array assignment (`array[index] = val`) and the heap position update (`val.heapPosition = index`) until the very end of the loop, significantly minimizing memory stores during priority queue rebalancing.
**Action:** Implemented the half-exchange pattern in `BinaryHeapOpenSet.java` `update()` and `removeLowest()` methods to eliminate redundant store operations.
