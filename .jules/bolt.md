...
## 2026-08-19 - AStar Innermost Loop Hoisting
**Learning:** In AStarPathFinder, invariant node calculations like `currentNode.x >> 4` and `currentNode.z >> 4` were being evaluated inside the hottest innermost `moves` loop multiple times per node. While JIT can often optimize this, explicitly hoisting these saves cycles immediately on initialization and is extremely valuable given the loop processes millions of times for long pathing calculations.
**Action:** Hoist node-specific calculations outside adjacent movement loops in AStar or other graph search algorithms.
