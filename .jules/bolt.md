...
## 2026-09-04 - AbstractNodeCostSearch bypass map lookup
**Learning:** In `AbstractNodeCostSearch` and `AStarPathFinder`, using `getNodeAtPosition` when `peekNodeAtPosition` returned null resulted in a redundant map lookup (`map.get()`) because both methods checked the map for the node.
**Action:** Replaced `getNodeAtPosition` with a new `createNodeAtPosition` method in `AStarPathFinder` to instantiate and map the new node directly, bypassing the redundant `map.get()`.
