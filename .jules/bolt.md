...
## 2025-05-21 - Binary Heap Half-Exchange Optimization
**Learning:** In the `BinaryHeapOpenSet` priority queue, the sift-up and sift-down operations were fully swapping the elements and their recorded heap positions at every step of the loop. This causes unnecessary array assignments (`array[index] = val`) and field updates (`val.heapPosition = index`) on elements that are just going to be shifted down/up again in the next iteration. This was noticed during A* pathfinder profiling.
**Action:** Implemented a half-exchange optimization where the assignment of the final array index and the `heapPosition` property for the element being shifted is deferred until the very end of the loop, saving multiple memory operations per heap update/insertion.
