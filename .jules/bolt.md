...
## 2026-06-17 - Priority Queue Half-Exchange Optimization
**Learning:** In the `BinaryHeapOpenSet` priority queue, the `siftUp` and `siftDown` operations were writing the swapped nodes to the backing array on every loop iteration, alongside updating the respective `heapPosition` references. This resulted in redundant array access overhead in the inner loops. The half-exchange optimization defers storing the item being sifted into the array and updating its heap position until its final array slot is determined, reducing write operations.
**Action:** Applied the half-exchange pattern in `BinaryHeapOpenSet`, minimizing array operations during pathfinding heap adjustments.
