...
## 2026-09-03 - Avoid Redundant Map Lookups in A*
**Learning:** In Baritone's AbstractNodeCostSearch and AStarPathFinder, when checking for node existence using peekNodeAtPosition (which calls map.get()), if the node is not found, calling a generic getNodeAtPosition method performs another redundant map.get() before inserting. This redundancy occurs in the innermost loop millions of times.
**Action:** Created createNodeAtPosition to directly instantiate and insert the new node into the map when peekNodeAtPosition returns null, bypassing the unnecessary map.get().
