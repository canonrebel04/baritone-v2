...
**Learning:** In Baritone's `BinaryHeapOpenSet`, both `update` (sift-up) and `removeLowest` (sift-down) perform full swaps inside the loop. This means memory stores happen unnecessarily inside the tightest loops of A* search.
**Action:** Implemented a half-exchange optimization to defer assignment of `array[index] = val` and `val.heapPosition = index` until the end of the loop, reducing memory writes per loop iteration from 4 to 2.
