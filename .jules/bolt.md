...
## 2026-06-05 - BinaryHeapOpenSet Half-Exchanges
**Learning:** `BinaryHeapOpenSet` uses an array-backed binary heap. Previously, `update` (sift-up) and `removeLowest` (sift-down) performed full swaps on every level of the tree. A full swap requires multiple memory stores (updating the array elements and their `heapPosition` references). For an A* pathfinder processing millions of nodes, this is a significant bottleneck. By using the "hole-bubbling" or half-exchange technique, we defer the final assignment to the very end of the loop, shifting elements rather than fully swapping them.
**Action:** Replaced full swaps in binary heap sift operations with half-exchanges to minimize memory writes.
