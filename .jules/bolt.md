...
**Learning:** Priority Queue operations (`sift-up` and `sift-down`) can perform multiple full swaps (three assignments) per level. A "half-exchange" optimization instead pulls the hole up/down and only writes the inserted/moved value once at its final destination. Since `BinaryHeapOpenSet` sits in the absolute hottest path of the A* search loop, saving two assignments per level in the heap translates to noticeable time savings when calculating long paths.
**Action:** Replaced full swaps in `BinaryHeapOpenSet.update` and `BinaryHeapOpenSet.removeLowest` with half-exchanges by deferring `array[index] = val` and `val.heapPosition = index` to the end of the loop.
