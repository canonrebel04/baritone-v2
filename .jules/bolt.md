...
## 2025-05-23 - BetterBlockPos.longHash could be BlockPos.asLong()
**Learning:** `BlockPos.asLong` combines coordinates into a single `long`, but its specific bit layout (Z in the middle) combined with Fastutil's internal mixing algorithm causes catastrophic Z-axis collisions in `Long2ObjectOpenHashMap`. `BetterBlockPos` provides `serializeToLong`, which guarantees a bijection (0 collisions) when mapped to `Long2ObjectOpenHashMap`. Given this is executed millions of times in `AStarPathFinder`, using `serializeToLong` rather than `BlockPos.asLong` provides a tremendous speedup by preserving perfect hashing.
**Action:** Replaced `BlockPos.asLong(x, y, z)` with `serializeToLong(x, y, z)` in `BetterBlockPos.longHash`.
