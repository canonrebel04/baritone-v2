...
## 2026-06-18 - Priority Queue Half-Exchange
**Learning:** The `BinaryHeapOpenSet` priority queue's sift-up and sift-down operations previously performed full array exchanges inside the loop. This resulted in redundant stores of the `val` element on every single swap iteration. Because this code sits in the absolute hottest path of the A* algorithm (invoked millions of times for pathfinding), these writes caused severe cache and memory pressure.
**Action:** Applied 'half-exchange' optimization in `update` and `removeLowest`, moving child/parent nodes by one array cell without writing `val` until the target index is determined at the very end. This reduces array memory stores by 50% in the loop.
